...
final class DunenBLEManager: NSObject, ObservableObject {
    ...
    private func updateDemo() {
        ...
        let wave = sin(demoTick / 5.0)
        let accelPulse = max(0, sin(demoTick / 2.2))
        let speed = max(0, 46 + 24 * wave + 9 * sin(demoTick / 1.45))
        let rpm = Int(speed * 82 + 650 + 450 * accelPulse)
        let voltage = 78.8 - min(demoTick / 1400.0, 4.0) - accelPulse * 0.35 + 0.15 * sin(demoTick / 3.0)
        let current = max(0, speed / 1.75 + accelPulse * 32)
        let modeCycle = Int((demoTick / 15).truncatingRemainder(dividingBy: 5))
        let modes: [RideMode] = [.eco, .xc, .sports, .park, .reverse]
        let mode = modes[modeCycle]

        telemetry.speedKmh = mode == .park ? 0 : (mode == .reverse ? min(speed, 14) : speed)
        telemetry.rpm = mode == .park ? 0 : rpm
        telemetry.voltage = voltage
        telemetry.currentA = mode == .park ? 0 : current
        ...
        telemetry.motorTemp = 33 + telemetry.speedKmh / 6 + current / 12
        telemetry.controllerTemp = 28 + current / 4.4
        telemetry.mode = mode
        telemetry.reverseActive = mode == .reverse
        telemetry.parkingActive = mode == .park
        telemetry.kickstandActive = mode == .park
        telemetry.brakeActive = accelPulse < 0.08 && telemetry.speedKmh > 10
